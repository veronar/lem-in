...
/*
** ft_linkback: links the pointed to node back to the current
** since the links go both ways so 0-1 is also 1-0
*/

static int	st_linkback(t_room **current, t_room **pointer)
{
	t_room	**temp;
	t_room	**new;
	int		size;
...
int			ft_linkarr(t_room **current, t_room **pointer)
{
	...
	if (temp == NULL)
	{
		...
		temp[1] = NULL;
		(*current)->links = temp;
		return (st_linkback(pointer, current));
	}
	while (temp[size] != NULL)
		size++;
	if (!(new = (t_room**)malloc(sizeof(t_room*) * (size + 2))))
